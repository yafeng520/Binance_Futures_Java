    ...
    public static void main(String[] args) {
        ...
        client.subscribeAggregateTradeEvent( symbol,
                ((event) -> {
                    ...
                    StepGrowthDirection stepGrowthDirection = analyzer.calculateStepGrowthDirection(currentPrice);
                    LinkedList<StepGrowth> rollingWindow = analyzer.getRollingWindow();
                    float lastStepGrowthDirection = rollingWindow.getLast().getStepGrowthRate();
                    rollingWindow.poll();
                    StepGrowth stepGrowth = new StepGrowth();
                    stepGrowth.setStepGrowthDirection(stepGrowthDirection);
                    stepGrowth.setQuantity(currentQuantity);
                    rollingWindow.offer(stepGrowth);
                    int curSideStepGrowthDirectionTotal = analyzer.calculateSideStepGrowthDirectionTotal(lastStepGrowthDirection);//rollingWindow内stepgrouth为正（负）的次数
                     ...
                     */
                    if (Math.abs(curSideStepGrowthDirectionTotal) > analyzer.getBuySideStepGrowthTotal()){//开始暴涨或暴跌，准备买入
                        if (curSideStepGrowthDirectionTotal > 0){//开始暴涨，买入做多
                        ...
                        }else if(curSideStepGrowthDirectionTotal < 0){//开始暴跌，买入做空
                    ...
                    if (Math.abs(curSideStepGrowthDirectionTotal) < analyzer.getSellSideStepGrowthTotal()){//开始恢复平静，准备卖出
