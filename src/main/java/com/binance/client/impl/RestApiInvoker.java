    ...
    try {
      if (json.containKey("success")) {
        ...
        if (!success) {
          ...
          if ("".equals(err_code)) {
            ...
            throw new BinanceApiException(
                BinanceApiException.EXEC_ERROR, "[Executing] " + err_code + ": " + err_msg);
          }
        }
        ...
        if (code != 200) {
          ...
          throw new BinanceApiException(
              BinanceApiException.EXEC_ERROR, "[Executing] " + code + ": " + message);
        }
      }
      ...
      throw new BinanceApiException(
          BinanceApiException.RUNTIME_ERROR, "[Invoking] Unexpected error: " + e.getMessage());
    }
  }
  ...
  static <T> T callSync(RestApiRequest<T> request) {
    try {
      ...
      if (response != null && response.body() != null) {
        ...
        throw new BinanceApiException(
            BinanceApiException.ENV_ERROR, "[Invoking] Cannot get the response from server");
      }
      ...
      throw new BinanceApiException(
          BinanceApiException.ENV_ERROR, "[Invoking] Unexpected error: " + e.getMessage());
    }
  }
