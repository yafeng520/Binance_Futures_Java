...
class InputChecker {
  ...
  InputChecker checkCurrency(String currency) {
    if (currency == null || "".equals(currency)) {
      throw new BinanceApiException(
          BinanceApiException.INPUT_ERROR, "[Input] Currency is mandatory");
    }
  ...
  InputChecker checkETF(String symbol) {
    if (!"hb10".equals(symbol)) {
      throw new BinanceApiException(
          BinanceApiException.INPUT_ERROR, "currently only support hb10 :-)");
    }
    return checkerInst;
  }
  ...
  InputChecker greaterOrEqual(Integer value, int base, String name) {
    if (value != null && value < base) {
      throw new BinanceApiException(
          BinanceApiException.INPUT_ERROR, "[Input] " + name + " should be greater than " + base);
    }
    return checkerInst;
  }
