...
public class JsonWrapperArray {
  ...
  public JsonWrapper getJsonObjectAt(int index) {
    if (array != null && array.size() > index) {
      ...
      throw new BinanceApiException(
          BinanceApiException.RUNTIME_ERROR, "[Json] Index is out of bound or array is null");
    }
  }
  ...
  public BigDecimal getBigDecimalAt(int index) {
    ...
    try {
      return new BigDecimal(
          new BigDecimal(getStringAt(index)).stripTrailingZeros().toPlainString());
    ...
    array.forEach(
        (object) -> {
          if (!(object instanceof JSONObject)) {
            throw new BinanceApiException(
                BinanceApiException.RUNTIME_ERROR, "[Json] Parse array error in forEach");
          }
    ...
    array.forEach(
        (object) -> {
          if (!(object instanceof JSONArray)) {
            throw new BinanceApiException(
                BinanceApiException.RUNTIME_ERROR, "[Json] Parse array error in forEachAsArray");
          }
    ...
    array.forEach(
        (object) -> {
          if (!(object instanceof String)) {
            throw new BinanceApiException(
                BinanceApiException.RUNTIME_ERROR, "[Json] Parse array error in forEachAsString");
          }
